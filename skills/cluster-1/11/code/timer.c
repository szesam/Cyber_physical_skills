...
static void i2c_task() //turn on alphanumeric display
{
    ...
    while (1) {
        ...
        xQueueReceive(timer_queue, &evt, portMAX_DELAY);
        // Create dummy structure to store structure from queue
        if (time_counter == 100) time_counter = 0;
        if (button_flag == 1)
        {
            printf("reset timer \n");
            button_flag = 0;
            displaybuffer[3] = 0b0000110000111111; //0
            displaybuffer[2] = 0b0000110000111111; //0
            displaybuffer[1] = 0b0000110000111111; //0
            displaybuffer[0] = 0b0000110000111111; //0
            time_counter = 0;
        ...
        if (evt.flag == 1)
        {
            displaybuffer[3] = alphafonttable[time_counter%10 + '0'];
            displaybuffer[2] = alphafonttable[(time_counter/10)%10 + '0'];
            displaybuffer[1] = alphafonttable[(time_counter/100)%10 + '0'];
            displaybuffer[0] = alphafonttable[(time_counter/1000)%10 + '0'];
            printf("%d\n", time_counter);
            // Send commands characters to display over I2C
            i2c_cmd_handle_t cmd4 = i2c_cmd_link_create();
            i2c_master_start(cmd4);
...
void app_main(void) {
    ...
    // Initiate alarm using timer API, initialize i2c, intialize button
    init();
    ...
    xTaskCreate(button_task, "button_task", 1024*4, NULL, configMAX_PRIORITIES-2, NULL);
